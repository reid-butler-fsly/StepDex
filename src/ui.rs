...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  header p { color: var(--muted); margin: 10px 0 0; max-width: 640px; }
  .powered { margin-top: 6px; font-size: 12px; color: var(--muted); }
  ...
  .bar { display: grid; grid-template-columns: 96px 1fr 34px; align-items: center; gap: 12px; font-size: 13px; }
  ...
  .btn {
    display:inline-block; margin-top:6px; font-size:13px; color: var(--neon);
    text-decoration:none; border:1px solid var(--panel-border); padding:8px 14px; border-radius:10px;
  }
...
<div class="wrap">
  <header>
    ...
    <p>Rank walkers by steps taken, then hand out cards from the collection — the more you walk,
       the rarer the card you earn. Ranking, rewards and insights are computed live at the edge.</p>
    <div class="powered">Computed on <b>Fastly Compute</b> · rarity from live market value · <a class="btn" href="/api/steps.csv" style="padding:2px 10px;margin-left:6px">Download CSV</a></div>
  </header>
  ...
  <footer>StepDex POC · in the spirit of EdgeWalk · steps → distance via a 0.74 m average stride.</footer>
</div>
...
function renderInsights() {
  ...
  document.getElementById("insights").innerHTML = [
    stat("Steps walked", fmt(d.total_steps), `${d.total_participants} walkers`),
    stat("Distance", miles(d.total_miles), "combined, @0.74m stride"),
...
function renderPodium() {
  ...
  document.getElementById("podium").innerHTML = order.filter(i => top[i]).map(i => {
    ...
    return `<div class="pod ${i===0?'first':''}" style="--glow:${glow}">
      ...
      <div class="steps">${fmt(e.steps)} <small>steps · ${miles(e.miles)}</small></div>
...
function renderBars() {
  ...
  document.getElementById("bars").innerHTML = dist.map(t => `
    <div class="bar">
  ...
  document.getElementById("showcase").innerHTML = DATA.entries
    .filter(e => e.reward).map(e => {
      ...
      return `<div class="poke" style="--glow:${r.tier_color}">
        <div class="pk-rank">#${e.rank} · ${e.display_name}</div>
...
renderInsights(); renderPodium(); renderBars(); renderRows(); renderShowcase();
</script>
</body>
</html>"##;
